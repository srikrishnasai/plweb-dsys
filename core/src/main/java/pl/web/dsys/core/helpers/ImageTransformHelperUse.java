    ...
    private boolean checkIsVertical(String imgCrop, String imgRotate) {
        try {
            if (imgCrop != null) {
                ...
                long ratio = (this.getInt(cropArr[2]) - this.getInt(cropArr[0])) / (this.getInt(cropArr[3]) - this.getInt(cropArr[1]));
        ...
        try {
            if (this.imgNode.hasNode(nodeName)) {
                ...
                if (node.hasProperty("imageCrop")) {
                    String imgRotate = node.hasProperty("imageRotate") ? node.getProperty("imageRotate").getString() : "0";
